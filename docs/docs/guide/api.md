...
| ---------------- | ------------------- | -------- | ------------------------------------------------------------ |
| toggleControls   | true \| false       | -        | Manipulate the display state of the player control, and the parameter passes a Boolean parameter to control hiding or displaying;<br />By default, the playback controls will be hidden according to the closeControlsTimer, and the advanced function `closeTimer | openTimer` can be used to close or open the timer, for example:<br />`mp.toggleControls(true).closeTimer();`<br />`mp.toggleControls(false).openTimer();` `new` |
