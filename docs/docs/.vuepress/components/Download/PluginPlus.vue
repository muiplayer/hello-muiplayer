<template>
  <main>
    <div class="show-entry" @click="showBuy">{{ $lang.indexOf("en") > -1 ? "Buy" : "购买授权" }}</div>
    <br />
    <div class="show-entry" @click="showDialog">{{ $lang.indexOf("en") > -1 ? "Download" : "插件下载" }}</div>

    <Dialog ref="_dialogImg" :autoClose="false" width="380px">
      <Pyament ref="_pyament" @close="$refs._dialogImg.close(true)" @success="downloadPlus" />
    </Dialog>
        ...
        <div class="w-body">
          ...
          <div class="s-tip s-item">
            ...
            <div style="margin-left: 8px">
              <span>【商品标码】专业版扩展组件需在平台进行购买</span>
            </div>
          </div>

          <div class="s-item">
            <span class="label">选择扩展组件：</span>
          ...
          <div class="s-item">
            <span class="label">请输入订单号：</span>
            <input v-model="form.orderSign" class="secret-input" type="text" placeholder="请输入21位订单号" />
          </div>
        </div>
...
import Pyament from "../Payment/Index.vue";
export default {
  props: ["type"],
  components: {
    Dialog,
  ...
  methods: {
    showBuy() {
      ...
      this.$nextTick(() => {
        this.$refs._pyament.init(this.type);
      });
    },
    showDialog(e) {
    ...
    downloadPlus(params = {}) {
      ...
      let fetch = async () => {
        ...
        let url = "https://muiplayer-plus-mtvipmsbhk.cn-shanghai.fcapp.run/download/professional";
        try {
          let res = await axios({
            method: "get",
            url: url,
...
.download-dialog {
  ...
  .dialog-wrapper {
    animation: dialogWrapperAnimation 0.48s;
    width: 740px;
    border-radius: 4px;
